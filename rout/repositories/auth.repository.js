...
const findGoogleUser = async ({ providerId, email }) => {
  ...
  const params = email ? [providerId, email] : [providerId];
  const result = await database.query(queryText, params);
  return result.rows[0] || null;
};

const createGoogleUser = async ({ prenom, nom, email, picture, roleType, providerId }) => {
  const result = await database.query(
    `
      ...
      RETURNING *
    `,
    [prenom, nom, email, picture, roleType, providerId]
  );
  return result.rows[0];
};

const attachGoogleProvider = async ({ userId, providerId, picture }) => {
  const result = await database.query(
    `
      UPDATE utilisateur
      SET provider_id = $1,
